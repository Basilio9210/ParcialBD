...
datafile 'MID_TERM_datafile' size 50M;

CREATE USER usrMID_TERM
IDENTIFIED BY usrMID_TERM
DEFAULT TABLESPACE MID_TERM
QUOTA 20M ON MID_TERM;

CREATE SEQUENCE COMUNAS_SEQ START WITH 50 INCREMENT BY 3 NOCYCLE;
CREATE SEQUENCE COLEGIOS_SEQ START WITH 100 INCREMENT BY 1 NOCYCLE;
...
ADD ID PRIMARY KEY;
 ...
 ALTER TABLE COMUNAS
    ADD CONSTRAINT COMUNA_FK FOREIGN KEY (ID) REFERENCES colegios (ID);  
...
SELECT  COUNT(BARRIO), BARRIO, NOMBRE_SEDE FROM COLEGIOS 
INNER JOIN COMUNAS ON  COLEGIOS.COMUNA_ID = COMUNAS.ID
WHERE COMUNAS.NOMBRE = 'BUENOS AIRES'
GROUP BY BARRIO, NOMBRE_SEDE;

SELECT COLEGIOS.ID, COLEGIOS.NOMBRE_SEDE, COLEGIOS.COMUNA_ID, COLEGIOS.BARRIO  FROM COLEGIOS
INNER JOIN COMUNAS ON COLEGIOS.ID = COMUNAS.ID;
